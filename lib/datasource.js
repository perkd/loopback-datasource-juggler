...
const {ModelRegistry, findEquivalentAnonymousModel} = require('./model-registry');
const {registerMongoObjectIdValidations} = require('./mongo-objectid-validation');
...
/**
 * Creates a property descriptor for the dataSource property that automatically
 * calls getDataSource() when available, providing multitenant support.
 *
 * This utility function eliminates code duplication across multiple locations
 * where dataSource property assignment occurs in the DataSource class.
 *
 * @param {Object} originalDataSource - The original datasource to use as fallback
 * @returns {Object} Property descriptor object for Object.defineProperty()
 */
...
function DataSource(name, settings, modelBuilder) {
  ...
  // Note: this.models is now defined as a property getter/setter below
 ...
 * Uses ModelRegistryProxy for clean separation of concerns and better maintainability
 * Provides O(1) model access while maintaining 100% backward compatibility
 */
Object.defineProperty(DataSource.prototype, 'models', {
  ...
  set: function(value) {
    ...
    delete this._modelRegistryProxy;

    // Handle backward compatibility - register any models that are being set
    if (value && typeof value === 'object') {
      ...
      Object.keys(value).forEach(modelName => {
        if (value[modelName] && typeof value[modelName] === 'object') {
          ...
          // Check if dataSource property already exists as a descriptor
          const existingDescriptor = Object.getOwnPropertyDescriptor(model, 'dataSource');
          if (existingDescriptor && existingDescriptor.set) {
            // Property exists with a setter, use it to update the datasource
            model.dataSource = originalDataSource;
            ...
            Object.defineProperty(model, 'dataSource',
              createDataSourcePropertyDescriptor(originalDataSource));
            ...
            // Property exists but is not configurable, just set the _originalDataSource
            // This handles edge cases where the property might be non-configurable
            model._originalDataSource = originalDataSource;
          }
          // Ensure model has correct name
          if (!model.modelName) {
...
DataSource.prototype.setupDataAccess = function(modelClass, settings) {
  ...
  registerMongoObjectIdValidations(modelClass, settings, {
...
DataSource.prototype.define = function defineClass(className, properties, settings) {
  ...
  // Define property accessor that calls getDataSource() when available
  Object.defineProperty(modelClass, 'dataSource', createDataSourcePropertyDescriptor(originalDataSource));

  if (settings.unresolved) {
    return modelClass;
  }

  // Register the model in the centralized ModelRegistry
  const {ModelRegistry} = require('./model-registry');
...
DataSource.prototype.attach = function(modelClass) {
  ...
  if (modelClass.modelBuilder !== this.modelBuilder) {
    ...
    // Check if this is an anonymous model
    if (modelClass.settings && modelClass.settings.anonymous) {
      // Try to find an equivalent model in our registry
      // The ModelRegistry will handle getting the tenant from context if needed
      const existingModel = ModelRegistry.findModelByStructure(
        modelClass.definition.properties,
      );

      if (existingModel) {
        debug('Using equivalent model %s instead of %s', existingModel.modelName, modelClass.modelName);
        return existingModel;
      }

      // If no equivalent model found, try the original implementation's approach
      const localExistingModel = findEquivalentAnonymousModel(this.modelBuilder, modelClass);

      if (localExistingModel) {
        // Register the model in our registry for future reuse
        // The ModelRegistry will handle getting the tenant from context if needed
        ModelRegistry.registerModel(localExistingModel, localExistingModel.definition.properties);
        return localExistingModel;
      }
    }
  ...
  // This ensures that dataSource.models proxy can find them (as specified in proposal)
  try {
    const {ModelRegistry} = require('./model-registry');
...
DataSource.prototype.disconnect = function disconnect(cb) {
  ...
  // Enhanced cleanup: Clean up ModelRegistryProxy immediately
  // Clean up ModelRegistryProxy to prevent memory leaks
  if (self._modelRegistryProxy) {
    delete self._modelRegistryProxy;
  }

  // Enhanced cleanup: Clean up tenant registry
  const cleanup = function() {
    // Clean up tenant registry with stable ID and DataSource reference
    if (self._dsId) {
      try {
        const {ModelRegistry} = require('./model-registry');
        ModelRegistry.cleanupTenant(`ds_${self._dsId}`, self);
      } catch (err) {
        // Silently ignore cleanup errors to prevent disconnect failures
        console.warn('Warning: Failed to cleanup ModelRegistry for DataSource:', err.message);
      }
    }
  };
  ...
  if (this.connected && (typeof this.connector.disconnect === 'function')) {
    this.connector.disconnect(function(err, result) {
      ...
      cleanup(); // Perform cleanup after successful disconnect
      cb && cb(err, result);
    });
  } else {
    process.nextTick(function() {
      self.connected = false;
      cleanup(); // Perform cleanup even for non-connected DataSources
      cb && cb();
    });
  }
  return cb.promise;
