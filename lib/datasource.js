...
DataSource.prototype.setupDataAccess = function(modelClass, settings) {
  // Handle nested properties with AnonymousModels
  if (modelClass.definition && modelClass.definition.properties) {
    ...
    for (const propName in properties) {
      ...
      // Check if the property type is a function (could be an AnonymousModel)
      if (typeof prop.type === 'function' &&
          prop.type.modelName &&
          prop.type.modelName.startsWith('AnonymousModel_')) {

        // Check if we already have an equivalent model in our ModelBuilder
        const existingModel = this.modelBuilder.findMatchingAnonymousModel(prop.type.definition.properties);

        if (existingModel && existingModel !== prop.type) {
          debug('Reusing existing AnonymousModel %s instead of creating a new one %s for property %s in model %s',
            existingModel.modelName, prop.type.modelName, propName, modelClass.modelName);

          // Replace the AnonymousModel with the existing one
          prop.type = existingModel;
        }
      }

      // Handle array types with AnonymousModels
      if (Array.isArray(prop.type) && prop.type.length > 0) {
        const itemType = prop.type[0];

        // Check if the array item type is an AnonymousModel
        if (typeof itemType === 'function' &&
            itemType.modelName &&
            itemType.modelName.startsWith('AnonymousModel_')) {

          // Check if we already have an equivalent model in our ModelBuilder
          const existingModel = this.modelBuilder.findMatchingAnonymousModel(itemType.definition.properties);

          if (existingModel && existingModel !== itemType) {
            debug('Reusing existing AnonymousModel %s instead of creating a new one %s for array property %s in model %s',
              existingModel.modelName, itemType.modelName, propName, modelClass.modelName);

            // Replace the AnonymousModel with the existing one
            prop.type[0] = existingModel;
          }
        }
      }
    }
...
DataSource.prototype.attach = function(modelClass) {
  ...
  if (modelClass.modelBuilder !== this.modelBuilder) {
    ...
    // If not an anonymous model or no equivalent found, proceed with normal attachment
...
/**
 * Find an equivalent anonymous model in the target ModelBuilder
 * @param {ModelBuilder} modelBuilder The target ModelBuilder
 * @param {Function} sourceModel The source model to find an equivalent for
 * @returns {Function|null} The equivalent model or null if not found
 * @private
 */
function findEquivalentAnonymousModel(modelBuilder, sourceModel) {
  // If the model is not anonymous, don't try to find an equivalent
  if (!sourceModel.settings || !sourceModel.settings.anonymous) {
    return null;
  }

  // Get all anonymous models from the target ModelBuilder
  const anonymousModels = Object.keys(modelBuilder.models)
    ...
    .filter(model => model.settings && model.settings.anonymous);

  // Compare the properties of each anonymous model with the source model
...
/**
 * Compare two property objects to determine if they are equivalent
 * @param {Object} props1 First property object
 * @param {Object} props2 Second property object
 * @returns {Boolean} True if the properties are equivalent
 * @private
 */
function arePropertiesEquivalent(props1, props2) {
  ...
  // If they have different number of properties, they're not equivalent
  if (keys1.length !== keys2.length) {
    return false;
  }

  // Check if all properties in props1 exist in props2 with the same type
  for (const key of keys1) {
    ...
    // Compare the type of the properties
    if (prop1.type !== prop2.type) {
      // If the types are functions, compare their names
      if (typeof prop1.type === 'function' && typeof prop2.type === 'function') {
    ...
    // For nested properties, recursively check
    if (prop1.type === 'object' && prop2.type === 'object') {
      if (!arePropertiesEquivalent(prop1.properties, prop2.properties)) {
        return false;
      }
    }
  }
