...
/**
 * Define the models property as a ModelRegistryProxy for centralized model management
 * This replaces the direct reference to modelBuilder.models with an intelligent proxy
 * that provides owner-aware filtering while maintaining 100% backward compatibility.
 */
...
DataSource.prototype.attach = function(modelClass) {
  ...
  // Register the newly attached model in our registry if it's anonymous
  if (modelClass.settings && modelClass.settings.anonymous) {
    // The ModelRegistry will handle getting the tenant from context if needed
