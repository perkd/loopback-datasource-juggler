...
// Set up debug
const debug = require('debug')('loopback:modelbuilder');
...
/**
 * Find an existing anonymous model that matches the given properties
 * @param {Object} properties The properties to match
 * @returns {Function|null} The matching model or null if not found
 * @private
 */
ModelBuilder.prototype.findMatchingAnonymousModel = function(properties) {
  // Get all anonymous models
  const anonymousModels = Object.keys(this.models)
    .filter(name => name.startsWith('AnonymousModel_'))
    .map(name => this.models[name])
    .filter(model => model.settings && model.settings.anonymous);

  // Compare the properties of each anonymous model with the given properties
  for (const model of anonymousModels) {
    if (arePropertiesEquivalent(model.definition.properties, properties)) {
      return model;
    }
  }
...
ModelBuilder.prototype.resolveType = function(prop, isSubProperty) {
  ...
  if (typeof prop === 'string') {
    ...
    if (!isSubProperty && prop.type) {
      ...
      // First, check if we already have a matching anonymous model
      const existingModel = this.findMatchingAnonymousModel(prop);
      if (existingModel) {
        debug('Reusing existing AnonymousModel %s for nested property', existingModel.modelName);
        return existingModel;
      }

      // If not, create a new anonymous model
      const modelName = this.getSchemaName(null);
      debug('Creating new AnonymousModel %s for nested property', modelName);
