...
function List(items, itemType, parent) {
  ...
  items.forEach(function(item, i) {
    if (itemType && !(item instanceof itemType)) {
      ...
      arr[i] = parent ? cloneParentedModelForParent(item, parent, itemType) : item;
    }
    // Apply parent reference to each item in the list
    if (parent && arr[i] && typeof arr[i] === 'object') {
      try {
        applyParentProperty(arr[i], parent);
      } catch (e) {
        // Ignore errors when applying parent property
        console.error('Error applying parent property:', e);
      }
    }
  });
...
List.prototype.push = function(obj) {
  const item = this.itemType && (obj instanceof this.itemType) ?
    cloneParentedModelForParent(obj, this.parent, this.itemType) :
    this.toItem(obj);

  // Apply parent reference to the item being pushed
  if (item && typeof item === 'object' && this.parent) {
    try {
      applyParentProperty(item, this.parent);
    } catch (e) {
      // Ignore errors when applying parent property
      console.error('Error applying parent property in push:', e);
    }
  }
