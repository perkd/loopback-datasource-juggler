...
/**
 * Hybrid Model Registry - Maintains backward compatibility while adding tenant awareness
 * 
 * Key design principles:
 * 1. Named models use global registry (existing behavior)
 * 2. Anonymous models use tenant-scoped registries (new behavior to prevent memory leaks)
 * 3. All existing APIs remain unchanged (100% backward compatibility)
 * 4. Automatic cleanup of inactive tenant registries
 */
const ModelRegistry = (() => {
  // Global storage for backward compatibility
  const globalModelsByFingerprint = new Map();
  const globalModelsByName = new Map();
  const globalTenantUsage = new Map(); // For debug tracking only

  // Tenant-scoped storage (new)
   ...
   * @returns {TenantRegistry|null} The tenant registry or null if invalid tenant
   */
  ...
  /**
   * Check if a model should use tenant-scoped registry
   * @param {Object} model - The model to check
   * @returns {Boolean} True if should use tenant registry
   */
  function shouldUseTenantRegistry(model) {
    // Only anonymous models use tenant registries to prevent memory leaks
    return model && model.settings && model.settings.anonymous;
  }
  ...
  return {
    ...
    registerModel(model, properties) {
      ...
      const useTenantRegistry = shouldUseTenantRegistry(model);

      if (useTenantRegistry && tenantCode) {
        ...
        if (tenantRegistry) {
          ...
          modelToTenant.set(model, tenantCode);
          
          // Also add to global maps for backward compatibility lookups
          ...
          // Fallback to global registry if tenant registry creation fails
          const fingerprint = this.generateFingerprint(properties || model.definition?.properties);
          globalModelsByFingerprint.set(fingerprint, model);
          globalModelsByName.set(model.modelName, model);
        }
      ...
      // Track tenant usage for debug purposes (existing behavior)
      if (debug.enabled && tenantCode) {
        if (!globalTenantUsage.has(tenantCode)) {
          globalTenantUsage.set(tenantCode, new Set());
        }
        globalTenantUsage.get(tenantCode).add(model.modelName);
      }

      // Track statistics
      totalModels++;

      debug(`Registered model ${model.modelName} (tenant: ${tenantCode || 'none'}, anonymous: ${useTenantRegistry})`);
    ...
    findModelByStructure(properties, currentModelBuilder) {
      ...
      // Try tenant registry first for anonymous models when tenant context is available
      if (tenantCode) {
        const tenantRegistry = getTenantRegistry(tenantCode);
        if (tenantRegistry) {
          ...
          if (model) {
            ...
            if (this.isEmbeddedModelStructure(properties)) {
              ...
              if (!this.areArraysEqual(propsKeys, modelPropsKeys)) {
                debug(`Skipping tenant model reuse for ${model.modelName} - property keys don't match exactly`);
            ...
            try {
              ...
              if (modelHasStrictEmbedded !== currentHasStrictEmbedded) {
                debug(`Skipping tenant model reuse for ${model.modelName} - strict settings don't match`);
                return null;
              }

              if (isTest && hasParentRef) {
                debug(`Skipping tenant model reuse for ${model.modelName} in test environment with parentRef enabled`);
                return null;
              }
            } catch (e) {
              debug('Error checking tenant model settings:', e);
              return null;
            }

            reuseCount++;
            
            // Track tenant usage for debug purposes
            if (debug.enabled) {
              if (!globalTenantUsage.has(tenantCode)) {
                globalTenantUsage.set(tenantCode, new Set());
              }
      ...
      if (model) {
        ...
        if (this.isEmbeddedModelStructure(properties)) {
          ...
          if (!this.areArraysEqual(propsKeys, modelPropsKeys)) {
            debug(`Skipping global model reuse for ${model.modelName} - property keys don't match exactly`);
            debug(`Original: ${JSON.stringify(propsKeys)}, New: ${JSON.stringify(modelPropsKeys)}`);
            return null;
          }
        }
        ...
        try {
          ...
          if (modelHasStrictEmbedded !== currentHasStrictEmbedded) {
            debug(`Skipping global model reuse for ${model.modelName} - strict settings don't match`);
            return null;
          }

          if (isTest && hasParentRef) {
            debug(`Skipping global model reuse for ${model.modelName} in test environment with parentRef enabled`);
            return null;
          }
        } catch (e) {
        ...
        debug(`Reusing global model ${model.modelName} for fingerprint ${fingerprint}`);
        return model;
      }
     ...
     */
    findModelByName(name) {
      // Check global registry first (covers both named models and anonymous models for backward compatibility)
      const globalModel = globalModelsByName.get(name);
      if (globalModel) {
        return globalModel;
      }

      // If not found globally, check tenant registry if tenant context is available
      const tenantCode = getCurrentTenant();
      if (tenantCode) {
        const tenantRegistry = getTenantRegistry(tenantCode);
        if (tenantRegistry) {
          return tenantRegistry.findModelByName(name);
        }
      }
    ...
    getStats() {
      ...
      return {
        ...
        tenantsTracked: debug.enabled ? globalTenantUsage.size : 'disabled',
        
        // New tenant registry statistics
        tenantRegistries: tenantRegistries.size,
        tenantStats: tenantStats,
        totalTenantModels: tenantStats.reduce((sum, stats) => sum + stats.modelCount, 0)
      };
    },

    /**
     * Clear the registry (mainly for testing)
     * ENHANCED: Now clears both global and tenant registries
     */
    clear() {
      globalModelsByFingerprint.clear();
      globalModelsByName.clear();
      globalTenantUsage.clear();
      ...
      totalModels = 0;
      reuseCount = 0;
      debug('Model registry cleared (global and tenant registries)');
    },
    ...
    cleanupTenant(tenantCode) {
      ...
      // Remove models from global registry that belong to this tenant
      ...
      if (globalTenantUsage.has(tenantCode)) {
        globalTenantUsage.delete(tenantCode);
      }
    ...
    getTenantRegistryInfo() {
      ...
      return {
        ...
        tenants: tenantInfo,
        globalModels: globalModelsByName.size
      };
    }
  };
})();
...
class RegistryManager {
   ...
   */
  forceCleanup() {
    return ModelRegistry.cleanupInactiveTenants(this.maxIdleTime);
  }
