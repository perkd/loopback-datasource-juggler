...
const ModelRegistry = (() => {
  ...
  function getTenantRegistry(tenantCode) {
    // Validate tenant code - should always be a DataSource-based tenant now
    if (!tenantCode || tenantCode === 'trap') {
      throw new Error(`Invalid tenant code: ${tenantCode}. All models must be associated with a DataSource.`);
    }
   ...
   * All models registered through DataSource have dataSource property set,
   * so we use DataSource instance identity for perfect tenant isolation.
   * @param {Object} model - The model to check
   * @param {String} currentTenant - The current tenant context (unused but kept for API compatibility)
   * @returns {String} The tenant to use for this model
   */
  function getEffectiveTenant(model, currentTenant) {
    // All models registered through DataSource.define() or DataSource.attach()
    // have model.dataSource set, so use DataSource instance for tenant isolation
    if (model && model.dataSource) {
      const dsId = model.dataSource._dsId || (model.dataSource._dsId = generateDataSourceId(model.dataSource));
      return `ds_${dsId}`;
    }

    // Handle temporary models created during model building process
    // These are internal models that don't have a DataSource yet
    if (model && (model.modelName?.startsWith('AnonymousModel_') || !model.modelName)) {
      return 'temp_models';
    }

    // This should never happen in normal operation since all models go through DataSource
    // But we keep it as a safety fallback
    throw new Error(`Model ${model?.modelName || 'unknown'} has no DataSource - cannot determine tenant`);
  }

  /**
   * Generate a unique identifier for a DataSource instance
   * @param {Object} dataSource - The DataSource instance
   * @returns {String} Unique identifier
   */
  function generateDataSourceId(dataSource) {
    // Use a combination of constructor name, connector type, and instance counter
  ...
  return {
    ...
    findModelByStructure(properties, currentModelBuilder) {
      ...
      // All models now use DataSource-based tenant isolation
      // Search only in the current tenant (DataSource-specific registry)
      const searchTenants = currentTenant ? [currentTenant] : [];
    ...
    getStats() {
      ...
      // All tenants are now DataSource-based, no special global tenant
     ...
     */
    hasModelForOwnerWithType(modelName, owner, ownerType) {
     ...
     */
    getModelForOwnerWithType(modelName, owner, ownerType) {
      if (!modelName || !owner || !ownerType) {
        return undefined;
      }
     ...
     * @param {Object} owner - The owner object to analyze
    ...
    _detectOwnerType(owner) {
      ...
      // Check for DataSource characteristics
      if (owner.connector !== undefined || owner.modelBuilder !== undefined ||
          (owner.constructor && owner.constructor.name === 'DataSource')) {
        return 'dataSource';
      }

      // Check for App characteristics
      if (owner.models !== undefined || owner.registry !== undefined ||
          (owner.constructor && owner.constructor.name === 'LoopBackApplication')) {
        return 'app';
      }
    ...
    getModelsForOwner(owner) {
      ...
      // Generate cache key using instance identity for proper isolation
      let cacheKey;
      if (owner.constructor.name === 'DataSource') {
        // Use DataSource instance identity for cache isolation
        const dsId = owner._dsId || (owner._dsId = generateDataSourceId(owner));
        cacheKey = `models_DataSource_${dsId}`;
      } else {
        // For other owners, use constructor name and name
        cacheKey = `models_${owner.constructor.name}_${owner.name || 'unnamed'}`;
      }
      ...
      if (!ownerType) {
        debug(`Unable to detect owner type for ${owner.constructor?.name || 'unknown'}`);
        return [];
      }
    ...
    /**
     * Clear the registry (primarily for testing) - PROPOSAL API
     */
    clear() {
      tenantRegistries.clear();
      modelToTenant.clear();
      ...
      totalModels = 0;
      reuseCount = 0;
      debug('ModelRegistry cleared');
    },

    /**
     * Get registry statistics for monitoring and debugging - PROPOSAL API
     * @returns {Object} Registry statistics
     */
    getStats() {
      return {
        totalModels,
        uniqueModels: tenantRegistries.size > 0 ?
          Array.from(tenantRegistries.values()).reduce((sum, registry) => sum + registry.modelsByName.size, 0) : 0,
        reuseCount,
        totalDataSources: tenantRegistries.size, // All tenants are now DataSource-based
