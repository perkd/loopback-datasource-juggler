...
const ModelRegistry = (() => {
  ...
  return {
    ...
    getModelsForOwner(owner, ownerType) {
      ...
      const currentTenant = getCurrentTenant();
      if (!currentTenant) {
        // No tenant context - search in global tenant only
        const globalRegistry = getTenantRegistry(GLOBAL_TENANT);
        const models = [];

        for (const model of globalRegistry.modelsByName.values()) {
      ...
      // With tenant context - search current tenant registry
    ...
    /**
     * Get model names for a specific owner
     * @param {Object} owner - The owner object (DataSource or App instance)
     * @param {String} ownerType - The type of owner ('dataSource' or 'app')
     * @returns {Array} Array of model names owned by the specified owner
     */
    getModelNamesForOwner(owner, ownerType) {
      return this.getModelsForOwner(owner, ownerType).map(model => model.modelName);
    },

    /**
     * Check if a model exists for a specific owner
    ...
    /**
     * Get a specific model for a specific owner
