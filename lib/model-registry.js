...
const ModelRegistry = (() => {
  ...
  function getEffectiveTenant(model, currentTenant) {
    ...
    debug(`Model ${model?.modelName || 'unknown'} has no DataSource, App, or tenant context, using global registry`);
    return 'global';
  }
  ...
  function generateAppId(app) {
    ...
    const appName = app.constructor?.name || app.name || 'App';
    const timestamp = Date.now();
    const random = Math.random().toString(36).substr(2, 9);
  ...
  return {
    ...
    registerModelForApp(app, model, properties) {
      ...
      debug(`Registered model ${model.modelName} for app ${app.constructor?.name || 'App'}`);
        ...
        if (model) {
          ...
          try {
            ...
            if (modelHasStrictEmbedded !== currentHasStrictEmbedded) {
              debug(`Skipping global model reuse for ${model.modelName} - strict settings don't match (model: ${modelHasStrictEmbedded}, current: ${currentHasStrictEmbedded})`);
