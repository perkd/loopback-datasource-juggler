...
function invokeConnectorMethod(connector, method, Model, args, options, cb) {
  ...
  if (!connector || (connector.dataSource == null &&
    Object.prototype.hasOwnProperty.call(connector, 'dataSource'))) {
    return process.nextTick(function() {
      cb(createDetachedConnectorError(method, Model.modelName));
    });
  }
...
DataAccessObject.getConnector = function() {
  ...
  if (!dataSource || !connector || (connector.dataSource == null &&
    Object.prototype.hasOwnProperty.call(connector, 'dataSource'))) {
    throw createDetachedConnectorError('getConnector', this.modelName);
  }
  return connector;
};
...
function stillConnecting(dataSource, obj, args) {
  ...
  if (!dataSource || !connector || (connector.dataSource == null &&
    ...
    const cb = typeof args[args.length - 1] === 'function' ?
      args[args.length - 1] : utils.createPromiseCallback();
    const method = args.callee && args.callee.name || 'operation';
    const modelName = obj && (obj.modelName ||
      (obj.constructor && obj.constructor.modelName)) || 'Model';

    process.nextTick(function() {
      cb(createDetachedConnectorError(method, modelName));
    });

    return typeof args[args.length - 1] === 'function' ? true : cb.promise;
  }
...
function createDetachedConnectorError(method, modelName) {
    ...
    if (!err && Array.isArray(data)) {
      async.map(data, function(item, next) {
        ...
        function buildResult(data, callback) {
          ...
          if (query && query.include) {
            if (query.collect) {
              ...
              includes.forEach(function(inc) {
                ...
                if (included) {
                  obj.__data[relationName] = included;

                  // Apply parent reference to embedded models
                  if (typeof included === 'object' && included !== null) {
                    if (Array.isArray(included) || included instanceof List) {
                      // Handle array/list of embedded models
                      for (let i = 0; i < included.length; i++) {
                        if (included[i] && typeof included[i] === 'object') {
                          // Only apply to anonymous models or when explicitly enabled
                          if (included[i].constructor &&
                              included[i].constructor.settings &&
                              (included[i].constructor.settings.anonymous ||
                               included[i].constructor.settings.parentRef === true)) {
                            included[i] = utils.cloneParentedModelForParent(included[i], obj);
                            utils.applyParentProperty(included[i], obj);
                          }
                        }
                      }
                    } else {
                      // Handle single embedded model - only apply to anonymous models or when explicitly enabled
                      if (included.constructor &&
                          included.constructor.settings &&
                          (included.constructor.settings.anonymous ||
                           included.constructor.settings.parentRef === true)) {
                        included = utils.cloneParentedModelForParent(included, obj);
                        obj.__data[relationName] = included;
                        utils.applyParentProperty(included, obj);
                      }
                    }
                  }
                }
          ...
          // Apply parent references to embedded models that are not part of relations
          if (obj && obj.__data) {
            for (const key in obj.__data) {
              const value = obj.__data[key];
                ...
                if (Array.isArray(value) || value instanceof List) {
                  ...
                  for (let i = 0; i < value.length; i++) {
                    if (value[i] && typeof value[i] === 'object') {
                      // Only apply to anonymous models or when explicitly enabled
                      if (value[i].constructor &&
                          value[i].constructor.settings &&
                          (value[i].constructor.settings.anonymous ||
                           value[i].constructor.settings.parentRef === true)) {
                        value[i] = utils.cloneParentedModelForParent(value[i], obj);
                        utils.applyParentProperty(value[i], obj);
                      }
                    }
                  }
                } else if (value.constructor && value.constructor !== Object) {
                  // Handle embedded model - only apply to anonymous models or when explicitly enabled
                  if (value.constructor.settings &&
                      (value.constructor.settings.anonymous ||
                       value.constructor.settings.parentRef === true)) {
                    obj.__data[key] = utils.cloneParentedModelForParent(value, obj);
                    utils.applyParentProperty(obj.__data[key], obj);
                  }
                }
              }
            }
...
DataAccessObject.prototype.getConnector = function() {
  ...
  if (!dataSource || !connector || (connector.dataSource == null &&
    Object.prototype.hasOwnProperty.call(connector, 'dataSource'))) {
    throw createDetachedConnectorError(
      'getConnector',
      this.constructor && this.constructor.modelName || 'Model',
    );
  }
  return connector;
};
