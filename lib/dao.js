...
DataAccessObject.prototype.setAttribute = function setAttribute(name, value) {
  if (name === 'external') {
    ...
    console.log('>>>>>>>>>>>>>>>>>>>>> AnonymousModel_4366: %j', inst.constructor.registry.modelBuilder?.models?.AnonymousModel_4366?.definition?.properties);
    console.log('>>>>>>>>>>>>>>>>>>>>> AnonymousModel_4367: %j', inst.constructor.registry.modelBuilder?.models?.AnonymousModel_4367?.definition?.properties);
  }
...
DataAccessObject.prototype.setAttributes = function setAttributes(data) {
  ...
  console.log('>>>>>>>>>>>>>>>>>>>>>', typeof inst.constructor?.registry?.modelBuilder?.models?.AnonymousModel_4367);
  Model.emit('set', inst);
};
