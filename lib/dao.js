...
DataAccessObject.applyProperties = function(data, inst) {
  console.log('debug [applyProperties-1] %j', {data, inst: inst?.toJSON()})
  let properties = this.definition.settings.properties;
  properties = properties || this.definition.settings.attributes;
  console.log('debug [applyProperties-2] %j', {properties})
...
DataAccessObject.prototype.setAttribute = function setAttribute(name, value) {
  if (name === 'external') {
    console.log('debug [setAttribute_1] value: %j, valueData: %j', value);
    if (this.constructor.name === 'Product') {
      console.log('>>>>>>>>>>>>>>>>>>>>> AnonymousModel_4366: %j', this.constructor.registry.modelBuilder?.models?.AnonymousModel_4366?.definition?.properties);
      console.log('>>>>>>>>>>>>>>>>>>>>> AnonymousModel_4367: %j', this.constructor.registry.modelBuilder?.models?.AnonymousModel_4367?.definition?.properties);
    } else if ( this.constructor.name === 'ProductImage') {
      console.log('>>>>>>>>>>>>>>>>>>>>> AnonymousModel_19: %j', this.constructor.registry.modelBuilder?.models?.AnonymousModel_19?.definition?.properties);
      console.log('>>>>>>>>>>>>>>>>>>>>> AnonymousModel_20: %j', this.constructor.registry.modelBuilder?.models?.AnonymousModel_20?.definition?.properties);
    }
  }
...
DataAccessObject.prototype.setAttributes = function setAttributes(data) {
  ...
  console.log('debug [setAttributes_applyProperties_1] data: %j', data);
  this.constructor.applyProperties(data, this);
  console.log('debug [setAttributes_applyProperties_2] data: %j', data);

  const Model = this.constructor;
  const inst = this;
  console.log('debug [setAttributes_applyProperties_3] inst: %j', inst?.toJSON());
  // update instance's properties
  for (const key in data) {
    inst.setAttribute(key, data[key]);
  }
  console.log('debug [setAttributes_applyProperties_4] inst: %j, external: %j', inst?.toJSON(), inst?.external);
  console.log('>>>>>>>>>>>>>>>>>>>>> %j', inst.constructor.registry.modelBuilder?.models?.AnonymousModel_4367?.definition?.properties);
  Model.emit('set', inst);
};
...
function(data, options, cb) {
  ...
  data = Model._sanitizeData(data, options);
  console.log('debug [updateAttributes_sanitizeData] data: %j', data);
  ...
  Model.notifyObserversOf('before save', context, function(err, ctx) {
    ...
    function validateAndSave(err, data) {
      console.log('debug [updateAttributes_validateAndSave_1] data: %j', data);
      if (err) return cb(err);
      let doValidate = true;
      if (options.validate === undefined) {
        if (Model.settings.automaticValidation !== undefined) {
      ...
      // update instance's properties
      try {
        console.log('debug [updateAttributes_setAttributes_1] inst: %j,  data: %j', inst?.toJSON(), data);
        inst.setAttributes(data);
        console.log('debug [updateAttributes_setAttributes_2] inst: %j,  data: %j', inst?.toJSON(), data);
      } catch (err) {
        return cb(err);
      }
      ...
      function triggerSave() {
        inst.trigger('save', function(saveDone) {
          inst.trigger('update', function(done) {
            console.log('debug [updateAttributes_triggerSave_1] data: %j', data);
            copyData(data, inst);
            const typedData = convertSubsetOfPropertiesByType(inst, data);
            console.log('debug [updateAttributes_triggerSave_2] typedData: %j', typedData);
            context.data = Model._sanitizeData(typedData, options);
            console.log('debug [updateAttributes_triggerSave_3] context.data: %j', context.data);
