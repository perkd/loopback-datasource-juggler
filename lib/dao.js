...
function(data, options, cb) {
  ...
  Model.notifyObserversOf('before save', context, function(err, ctx) {
    ...
    function validateAndSave(err, data) {
      ...
      try {
        inst.setAttributes(data);
        console.log('debug [updateAttributes_setAttributes] inst: %j', inst?.toJSON());
      } catch (err) {
        return cb(err);
      }
      ...
      function triggerSave() {
        inst.trigger('save', function(saveDone) {
          inst.trigger('update', function(done) {
            copyData(data, inst);
            const typedData = convertSubsetOfPropertiesByType(inst, data);
            console.log('debug [updateAttributes_triggerSave_1] typedData: %j', typedData);
            context.data = Model._sanitizeData(typedData, options);
            console.log('debug [updateAttributes_triggerSave_2] context.data: %j', context.data);
