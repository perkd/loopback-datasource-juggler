...
DataAccessObject.prototype.setAttribute = function setAttribute(name, value) {
  if (name === 'external') {
    ...
    const modelNameExternal = this.external?.constructor?.name
    const modelNameShopify = this.external?.shopify?.constructor?.name

    if (!modelNameExternal || !modelNameShopify) {
      console.log('>>>>>>>>>>>>>>>>>>>>> MODEL MISSING: %s', modelNameExternal);
      console.log('>>>>>>>>>>>>>>>>>>>>> MODEL MISSING: %s', modelNameShopify);
    } else {
      console.log('>>>>>>>>>>>>>>>>>>>>> %s: %j', modelNameExternal, this.constructor.registry.modelBuilder?.models?.[modelNameExternal]?.definition?.properties);
      console.log('>>>>>>>>>>>>>>>>>>>>> %s: %j', modelNameShopify, this.constructor.registry.modelBuilder?.models?.[modelNameShopify]?.definition?.properties);
    }
  }
